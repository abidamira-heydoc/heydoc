    ...
    <>
        ...
        <div className="p-2 space-y-1">
          {conversations.map((conversation) => (
            <div key={conversation.id} className="relative group">
              ...
              <div className="absolute end-2 top-3 flex items-center gap-1">
          ...
          {conversations.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-xs mt-1">{t('sidebar.startNew')}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-4 border-t border-green-100 space-y-2 bg-white/50">
        ...
        {isPlatformAdmin && (
          ...
          >
            <svg className="w-5 h-5 me-3 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
            <span className="text-sm font-medium text-blue-700">{t('sidebar.platformDashboard')}</span>
          </button>
        )}
        ...
        {isAdmin && (
          ...
          >
            <svg className="w-5 h-5 me-3 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
            </svg>
            <span className="text-sm font-medium text-purple-700">{t('sidebar.orgDashboard')}</span>
          </button>
        )}
        <button
        ...
        >
          <svg className="w-5 h-5 me-3 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
          </svg>
          <span className="text-sm font-medium text-gray-700">{t('sidebar.healthProfile')}</span>
        </button>
        <button
